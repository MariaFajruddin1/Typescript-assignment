...
class SMS {
  ...
  logIn(studentId: number, password: string): {status: boolean, msg: string} {
    ...
    if (!student) {
      return {
        status: false,
        msg: chalk.red("Student do not exist")
      }
    }
    ...
    if (student.password == password) {
      ...
      return {
        status: true,
        msg: (chalk.green("Successfully Logged In"))
      }
    }
    else {
      return {
        status: false,
        msg: chalk.red("Incorrect password")
      }
  ...
  setPassword(studentId: number, newPassword: string, oldPassword: string) {
    ...
    if (!student) throw Error(chalk.red("Student do not exist"));
    if (!student.loginStatus) throw Error(chalk.red("Not logged In"));
    ...
    if (student.password === oldPassword) {
      ...
      throw Error(chalk.red("Incorrect password"));
    }
  ...
  enrollInCourses(studentId: number, courseCodes: string[]) {
    ...
    let student = this.students.get(studentId);
    if (!student) throw Error(chalk.red("Student do not exist"));
    if (!student.loginStatus) throw Error(gradient.rainbow("You are not logged In"));
    ...
    if(student.balance < Number(totalFeeToPay)){
      throw Error(chalk.red("You don't have enough balance to enroll in these courses"));
    }

    courseCodes.forEach((code) => {
      const course = this.courses.get(code)
      if (!course) throw Error(chalk.red(`course do not exist ${code}`));
...
export default new SMS(gradient.rainbow(figlet.textSync("Welcome  To  My  SMS ")));