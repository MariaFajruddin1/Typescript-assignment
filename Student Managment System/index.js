...
const handleQuit = () => {
    quit = true;
    console.log(gradient.rainbow("Bye. See you later"));
};
const handleLogOut = () => {
    console.clear();
...
const loggedInWelcomMsg = () => {
    if (studentData) {
        console.clear();
        console.log(gradient.rainbow(figlet.textSync("Welcome  To  My  SMS")));
        console.log("");
        console.table([
            {
...
const editProfile = async () => {
    ...
    await updatingComponent(chalk.yellow("Updating Profile"));
    loggedInWelcomMsg();
};
const enrollInACourse = async () => {
    if (studentData) {
        loggedInWelcomMsg();
        console.log(chalk.green("Already Enrolled In Courses:"));
        ...
        console.log(chalk.green("(See detail in courses section) Available Courses:"));
        ...
        console.log(chalk.blue("Total Fee for selected courses: ", totalfee));
        console.log("");
        if (totalfee > studentData.balance) {
            console.log(chalk.red("You don't have enough Balance for this transaction"));
        }
        else {
            const ans = await inquirer.prompt([{
                    name: "course",
                    type: "confirm",
                    message: gradient.rainbow(`Your balance is ${studentData.balance} and this will cost you ${totalfee}`)
                }]);
            if (ans.course) {
...
const updatePassword = async () => {
    ...
    console.log("ans: ", ans);
    console.log(chalk.blue("studentData.password: ", studentData.password));
    if (ans.oldPassword !== studentData.password) {
        console.log(chalk.red("Password not matched with old password"));
    }
    else if (ans.newPassword !== ans.confimredNewPassword) {
        console.log(chalk.red("Confirmed password not matched with new password"));
    }
    else {
        PIAIC_SMS.setPassword(studentData.studentId, ans.newPassword, studentData.password);
        await updatingComponent(chalk.yellow(`Updating Password`));
        loggedInWelcomMsg();
    }
};
