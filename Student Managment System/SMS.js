...
class SMS {
    ...
    setPassword(studentId, newPassword, oldPassword) {
        let student = this.students.get(studentId);
        if (!student)
            throw Error(chalk.red("Student do not exist"));
        if (!student.loginStatus)
            throw Error(chalk.red("Not logged In"));
    ...
    enrollInCourses(studentId, courseCodes) {
        ...
        if (!student)
            throw Error(chalk.red("Student do not exist"));
        if (!student.loginStatus)
            throw Error(gradient.rainbow("You are not logged In"));
        let totalFeeToPay = 0;
        ...
        courseCodes.forEach((code) => {
            const course = this.courses.get(code);
            if (!course)
                throw Error(chalk.red(`course do not exist ${code}`));
            if (student) {
